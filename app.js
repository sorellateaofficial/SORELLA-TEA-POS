...
function offlineMutation(path,method,body) {
  ...
  if(maps[resource]&&method==="POST"){const arr=state[maps[resource]];const row={...(body||{}),id:rid||body?.id||id(resource.slice(0,4).toUpperCase()+"_")};localUpsert(arr,row);queueMutation(path,method,body);saveOfflineSnapshot();return row;}
...
async function init() {
  loadCart();
  setSyncStatus(navigator.onLine===false);
